...
class DatasetWrapper1(TorchDataset):
    def __init__(self, data_source, input_size, transform=None, is_train=False,
        ...
        self.k_tfm = k_tfm if is_train else 1
        self.return_img0 = return_img0
        self.muwu_to_tensor = transforms.ToTensor()
        ...
        if self.transform is not None:
            for i in range(2):
                if isinstance(self.transform, (list, tuple)):
                    ...
                    img = self._transform_image(self.transform, img0)
                    output[f'img{i+1}'] = img

        if self.return_img0:
            output['img0'] = self.to_tensor(img0)

        return output['img1'], output['label'], output['img2']
...
class DatasetWrapper3(TorchDataset):
    def __init__(self, data_source, input_size, transform=None, is_train=False,
                 return_img0=False, k_tfm=1):
        self.data_source = data_source
        self.transform = transform # accept list (tuple) as input
    ...
    def __getitem__(self, idx):
        ...
        ascii_values = [ord(c) for c in output['impath']]
        ascii_values += [0] * (200 - len(output['impath']))
        return output['img'], output['label'], torch.tensor(ascii_values)
