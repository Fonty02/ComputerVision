...
\begin{figure*}[t!]
    \centering
    \includegraphics[width=\linewidth,height=0.2\textheight]{../img/diagram.png}
...
\begin{figure*}[t!]
    \centering
    \includegraphics[width=\linewidth,height=0.12\textheight]{../img/episode_results.png}
    \caption{Example of and episode using RN50 backbone}
\end{figure*}
...
Our comprehensive evaluation reveals that integrating explicit style-aware features does not improve few-shot artist classification performance. Standard CLIP with RN50 backbone consistently outperforms StyCLIP variants across all configurations, achieving 73.46\% accuracy in 16-shot scenarios versus StyCLIP's best performance of 48.75\%—a statistically significant 25 percentage point difference.

Key findings include: (1) TIMO and TIMO\_S demonstrate superior performance with no significant differences when using standard RN50, (2) accuracy improves substantially with more support examples (45.96\% to 73.46\% from 1-shot to 16-shot), and (3) StyCLIP variants consistently underperform across all conditions, suggesting that explicit Gram matrix-based style modeling interferes with pre-trained semantic representations rather than enhancing them.

The results indicate that CLIP's pre-trained features already capture sufficient stylistic information for artist classification, and that explicit style integration through architectural modifications may disrupt the learned semantic-visual alignment.
