...
The fundamental insight behind neural style transfer lies in the separation of content and style representations within convolutional neural networks. Content information, which describes the structural and semantic aspects of an image, is primarily captured by the spatial arrangement of feature activations in deeper layers. Style information, representing textural patterns, color distributions, and artistic characteristics, is encoded in the correlations between feature maps across different channels.

Gram matrices serve as the mathematical foundation for capturing these style correlations. Given a feature map $F^l \in \mathbb{R}^{C_l \times H_l \times W_l}$ from layer $l$ with $C_l$ channels and spatial dimensions $H_l \times W_l$, the Gram matrix $G^l \in \mathbb{R}^{C_l \times C_l}$ is computed as:

\[
G^l_{i,j} = \sum_{k=1}^{H_l} \sum_{m=1}^{W_l} F^l_{i,k,m} \cdot F^l_{j,k,m}
\]

This computation measures the inner product between vectorized feature maps of channels $i$ and $j$, effectively capturing how often different visual patterns co-occur spatially. The resulting Gram matrix encodes second-order statistics that are invariant to spatial location but sensitive to the distribution and correlation of visual features.

The normalization of Gram matrices is typically performed by dividing by the total number of spatial locations $(H_l \times W_l)$ to ensure scale invariance:

\[
G^l_{i,j} = \frac{1}{H_l \times W_l} \sum_{k=1}^{H_l} \sum_{m=1}^{W_l} F^l_{i,k,m} \cdot F^l_{j,k,m}
\]

This normalization allows style representations to be independent of image size and enables consistent style transfer across images of different resolutions.

While originally developed for style transfer applications, Gram matrix representations offer valuable insights into modeling artistic style for classification tasks. The ability to capture style information independently of content makes them particularly suitable for artist identification, where stylistic consistency across diverse subjects is a key discriminative factor.
...
The emergence of large-scale vision-language models like CLIP \cite{CLIP} has revolutionized few-shot artistic recognition by leveraging textual descriptions of artistic styles. CLIP learns multimodal representations by maximizing cosine similarity between correct image-text pairs while minimizing similarity for incorrect pairs through contrastive learning.

The CLIP architecture consists of visual and text encoders that project inputs into a shared embedding space. The visual encoder can be Vision Transformer (ViT) or ResNet-based (like RN50), while the text encoder uses Transformer architecture with self-attention mechanisms. Both encoders map to a common D-dimensional space (typically 512 or 1024 dimensions), enabling zero-shot classification through embedding similarity comparison.
...
In this section, we describe the experimental setup used to create, train, StyCLIP, and evaluate the models.
All these experiments were conducted on a single NVIDIA RTX 3060 GPU with 12GB of memory, 16GB of 3200MHz DDR4 RAM and Ryzen 5 3600 CPU.

\subsection{StyCLIP structure}

StyCLIP extends the standard CLIP architecture by integrating explicit style representations through Gram matrix features, specifically designed to capture artistic style information that complements semantic understanding. This section details the architectural components and modifications introduced in our approach.

\subsubsection{Standard CLIP with RN50 Backbone}

The baseline CLIP model with ResNet-50 backbone consists of:
\begin{itemize}
    \item \textbf{Visual Encoder}: To extract visual features from images into a 1024-dimensional embedding space
    \item \textbf{Text Encoder}: To extract semantic features from text descriptions into a 1024-dimensional embedding space
...
\begin{figure*}[t!]
    \centering
    \includegraphics[width=0.6\linewidth,height=0.15\textheight]{../img/StyCLIP.png}
    \caption{StyCLIP Architecture Overview}
\end{figure*}
\begin{itemize}
    \item \textbf{Gram Matrix Extraction Module}: Extracts style-specific correlations from intermediate visual features of the ResNet-50 backbone to capture stylistic patterns at different resolution scales. The resulting matrices represent channel correlations that encode texture information, brushstrokes, and artistic characteristics independent of semantic content.
    
    \item \textbf{Style Feature Projection Layers}: Perform dimensionality reduction of high-dimensional Gram matrix representations. They use a Multi-Layer Perceptron (MLP) to project concatenated Gram matrices (after extracting the upper triangular part for computational efficiency) from a high-dimensional space to a compact 256-dimensional space, preserving the most relevant stylistic information for artist classification.
    
    \item \textbf{Fusion Adapter}: Integrates CLIP semantic features and stylistic features through a bottleneck architecture. The module concatenates CLIP visual features (512 dimensions) with projected stylistic features (256 dimensions) and uses a bottleneck MLP adapter that reduces dimensionality (768 → bottleneck dimension hyperparameter → 512) to effectively fuse semantic and stylistic information into a unified representation optimized for few-shot artist classification.
\end{itemize}
...
\begin{figure*}[t!]
    \centering
    \includegraphics[width=0.6\linewidth,height=0.15\textheight]{../img/episode_example.drawio.png}
    \caption{Example of and 10 Way 4-Shot Episode}
\end{figure*}
...
\begin{itemize}
    ...
    \item \textbf{StyCLIP FSL1}: StyCLIP model with ResNet-50 backbone trained with 1-shot configuration
    \item \textbf{StyCLIP FSL4}: StyCLIP model with ResNet-50 backbone trained with 4-shot configuration
\end{itemize}
...
\midrule
APE & StyCLIP\_FSL1 & 2.05 & 0.93 & 2.05 & 0.87 \\
APE & StyCLIP\_FSL4 & 2.48 & 0.81 & 2.44 & 0.84 \\
APE & RN50 & 44.31 & 44.42 & 43.94 & 39.76 \\
GDA\_CLIP & StyCLIP\_FSL1 & 13.49 & 13.88 & 13.23 & 11.98 \\
GDA\_CLIP & StyCLIP\_FSL4 & 16.33 & 17.48 & 16.08 & 14.62 \\
GDA\_CLIP & RN50 & 41.41 & 38.86 & 40.76 & 35.33 \\
TIMO & StyCLIP\_FSL1 & 6.97 & 6.88 & 6.81 & 6.13 \\
TIMO & StyCLIP\_FSL4 & 7.37 & 7.57 & 7.17 & 6.69 \\
TIMO & RN50 & 43.88 & 44.32 & 43.52 & 40.29 \\
TIMO\_S & StyCLIP\_FSL1 & 7.28 & 7.27 & 7.11 & 6.38 \\
TIMO\_S & StyCLIP\_FSL4 & 7.31 & 7.56 & 7.13 & 6.63 \\
TIMO\_S & RN50 & \textbf{45.96} & \textbf{44.47} & \textbf{45.54} & \textbf{41.07} \\
Tip\_Adapter & StyCLIP\_FSL1 & 1.31 & 0.03 & 1.34 & 0.06 \\
Tip\_Adapter & StyCLIP\_FSL4 & 1.13 & 0.01 & 1.02 & 0.03 \\
...
\textbf{Model} & \textbf{Backbone} & \textbf{ACC} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
APE & StyCLIP\_FSL1 & 3.55 & 1.51 & 3.51 & 1.51 \\
APE & StyCLIP\_FSL4 & 3.39 & 2.03 & 3.38 & 1.71 \\
APE & RN50 & 47.09 & 47.77 & 46.65 & 42.37 \\
GDA\_CLIP & StyCLIP\_FSL1 & 20.95 & 21.79 & 20.59 & 19.40 \\
GDA\_CLIP & StyCLIP\_FSL4 & 25.96 & 27.35 & 25.48 & 24.22 \\
GDA\_CLIP & RN50 & 47.22 & 47.90 & 46.41 & 42.47 \\
TIMO & StyCLIP\_FSL1 & 12.14 & 11.82 & 11.87 & 10.78 \\
TIMO & StyCLIP\_FSL4 & 13.55 & 13.67 & 13.21 & 12.16 \\
TIMO & RN50 & 47.89 & 48.43 & 47.32 & 43.72 \\
TIMO\_S & StyCLIP\_FSL1 & 13.76 & 13.84 & 13.45 & 12.44 \\
TIMO\_S & StyCLIP\_FSL4 & 15.38 & 15.69 & 15.02 & 13.92 \\
TIMO\_S & RN50 & \textbf{48.65} & \textbf{48.78} & \textbf{47.95} & \textbf{43.98} \\
Tip\_Adapter & StyCLIP\_FSL1 & 1.19 & 0.02 & 1.19 & 0.04 \\
Tip\_Adapter & StyCLIP\_FSL4 & 1.19 & 0.02 & 1.08 & 0.03 \\
...
\textbf{Model} & \textbf{Backbone} & \textbf{ACC} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
APE & StyCLIP\_FSL1 & 4.62 & 2.09 & 4.42 & 2.17 \\
APE & StyCLIP\_FSL4 & 5.29 & 2.75 & 5.12 & 2.62 \\
APE & RN50 & 53.88 & 54.93 & 53.72 & 49.73 \\
GDA\_CLIP & StyCLIP\_FSL1 & 27.16 & 28.56 & 26.84 & 25.48 \\
GDA\_CLIP & StyCLIP\_FSL4 & 35.17 & 38.35 & 34.79 & 33.74 \\
GDA\_CLIP & RN50 & 55.54 & 58.24 & 54.97 & 52.66 \\
TIMO & StyCLIP\_FSL1 & 21.77 & 21.38 & 21.35 & 19.75 \\
TIMO & StyCLIP\_FSL4 & 23.43 & 25.00 & 23.06 & 22.13 \\
TIMO & RN50 & 56.48 & 59.14 & 55.90 & 53.96 \\
TIMO\_S & StyCLIP\_FSL1 & 23.24 & 23.04 & 22.81 & 21.19 \\
TIMO\_S & StyCLIP\_FSL4 & 26.36 & 28.69 & 25.97 & 25.27 \\
TIMO\_S & RN50 & \textbf{57.28} & \textbf{59.77} & \textbf{56.74} & \textbf{54.55} \\
Tip\_Adapter & StyCLIP\_FSL1 & 1.38 & 0.04 & 1.40 & 0.08 \\
Tip\_Adapter & StyCLIP\_FSL4 & 1.28 & 0.02 & 1.19 & 0.04 \\
...
\textbf{Model} & \textbf{Backbone} & \textbf{ACC} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
APE & StyCLIP\_FSL1 & 5.35 & 2.57 & 5.27 & 2.57 \\
APE & StyCLIP\_FSL4 & 7.52 & 4.71 & 7.35 & 4.42 \\
APE & RN50 & 58.10 & 58.94 & 57.97 & 54.03 \\
GDA\_CLIP & StyCLIP\_FSL1 & 34.25 & 35.76 & 33.93 & 32.55 \\
GDA\_CLIP & StyCLIP\_FSL4 & 46.97 & 50.15 & 46.42 & 45.55 \\
GDA\_CLIP & RN50 & 63.61 & 67.18 & 63.19 & 61.57 \\
TIMO & StyCLIP\_FSL1 & 30.64 & 30.61 & 30.49 & 28.71 \\
TIMO & StyCLIP\_FSL4 & 38.38 & 40.46 & 37.90 & 36.52 \\
TIMO & RN50 & \textbf{65.26} & \textbf{68.43} & \textbf{64.93} & \textbf{63.66} \\
TIMO\_S & StyCLIP\_FSL1 & 32.17 & 33.14 & 31.92 & 30.49 \\
TIMO\_S & StyCLIP\_FSL4 & 40.89 & 43.57 & 40.41 & 39.13 \\
TIMO\_S & RN50 & 65.08 & 68.25 & 64.75 & 63.45 \\
Tip\_Adapter & StyCLIP\_FSL1 & 1.71 & 0.21 & 1.72 & 0.24 \\
Tip\_Adapter & StyCLIP\_FSL4 & 1.38 & 0.03 & 1.28 & 0.06 \\
...
\textbf{Model} & \textbf{Backbone} & \textbf{ACC} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
APE & StyCLIP\_FSL1 & 6.73 & 3.35 & 6.55 & 3.37 \\
APE & StyCLIP\_FSL4 & 7.98 & 5.63 & 7.87 & 5.13 \\
APE & RN50 & 62.17 & 63.65 & 61.95 & 58.36 \\
GDA\_CLIP & StyCLIP\_FSL1 & 41.19 & 42.45 & 41.00 & 39.13 \\
GDA\_CLIP & StyCLIP\_FSL4 & 53.36 & 55.25 & 52.86 & 51.64 \\
GDA\_CLIP & RN50 & 73.06 & 76.12 & 72.58 & 71.63 \\
TIMO & StyCLIP\_FSL1 & 37.00 & 37.83 & 36.87 & 34.90 \\
TIMO & StyCLIP\_FSL4 & 43.27 & 45.16 & 42.80 & 41.36 \\
TIMO & RN50 & 73.36 & 76.59 & 72.94 & 72.09 \\
TIMO\_S & StyCLIP\_FSL1 & 39.20 & 40.52 & 39.01 & 37.22 \\
TIMO\_S & StyCLIP\_FSL4 & 48.75 & 50.58 & 48.27 & 46.90 \\
TIMO\_S & RN50 & \textbf{73.46} & \textbf{76.52} & \textbf{73.04} & \textbf{72.10} \\
Tip\_Adapter & StyCLIP\_FSL1 & 2.63 & 0.70 & 2.60 & 0.74 \\
Tip\_Adapter & StyCLIP\_FSL4 & 1.77 & 0.07 & 1.69 & 0.13 \\
...
\caption{Statistical Comparison Matrix for 1-Shot Classification (Accuracy \%). * indicates significant differences (p < 0.05) between models using t-test.}
...
\tiny
\captionsetup{width=\linewidth, font=scriptsize, justification=centering}
\caption{Statistical Comparison Matrix for 2-Shot Classification (Accuracy \%). * indicates significant differences (p < 0.05) between models using t-test.}
...
TIMO\_S\_RN50 & 45.11* & 45.26* & 1.56* & 27.71* & 22.69* & 4.56* & 38.99* & 38.59* & 2.08 & 38.69* & 38.65* & 0.00 & 44.65* & 44.83* & 7.31* \\
...
\tiny
\captionsetup{width=\linewidth, font=scriptsize, justification=centering}
\caption{Statistical Comparison Matrix for 4-Shot Classification (Accuracy \%). * indicates significant differences (p < 0.05) between models using t-test.}
...
Tip\_FSL1 & -3.24* & -3.91* & -52.51* & -25.78* & -33.79* & -54.16* & -10.95* & -12.35* & -55.11* & -21.87* & -24.98* & -55.90* & 0.00 & 0.09 & -43.58* \\
Tip\_FSL4 & -3.33* & -4.01* & -52.60* & -25.87* & -33.88* & -54.25* & -10.49* & -12.14* & -55.20* & -21.96* & -25.08* & -55.99* & -0.09 & 0.00 & -43.67* \\
...
\tiny
\captionsetup{width=\linewidth, font=scriptsize, justification=centering}
\caption{Statistical Comparison Matrix for 8-Shot Classification (Accuracy \%) .* indicates significant differences (p < 0.05) between models using t-test.}
...
\tiny
\captionsetup{width=\linewidth, font=scriptsize, justification=centering}
\caption{Statistical Comparison Matrix for 16-Shot Classification (Accuracy \%). * indicates significant differences (p < 0.05) between models using t-test.}
