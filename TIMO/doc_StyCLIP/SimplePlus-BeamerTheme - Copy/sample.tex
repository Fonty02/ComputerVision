...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \textbf{Challenge:} Few-shot visual artist classification -- identifying artistic styles with limited labeled examples per artist.
    
    \item \textbf{Problem:} 
      \begin{itemize}
        \item Artistic style relies on subtle visual cues (brushstrokes, color palettes, composition)
        \item CLIP-based methods excel at semantics but may miss fine-grained stylistic nuances
      \end{itemize}
...
\begin{frame}
  ...
  \begin{block}{Two Main Branches}
      \textbf{Text-Guided Image (TGI):}Uses textual representations to guide visual features      
      \textbf{Image-Guided Text (IGT):}Finds optimal weights for textual prompts
  \end{block}
   \begin{figure}[H]
      \centering
      \includegraphics[width=0.8\textwidth ,height=0.4\textheight]{../img/TIMO.png}
