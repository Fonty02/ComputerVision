...
\begin{frame}
  ...
  \begin{itemize}
    \item \textbf{Core Idea:} Extend CLIP with explicit style-aware features via Gram matrices to capture artist-specific patterns
 ...
 \item \textbf{Gram Matrix Extraction:} Extract style correlations from ResNet-50 intermediate layers
        ...
        \begin{equation*}
          \mathbf{s} = \text{MLP}(\text{vec}(\text{triu}(G^{(l)}))) \in \R^{d_s}
        \end{equation*}
        
        \item \textbf{Adaptive Fusion:} Integrate semantic and style features via bottleneck adapter
        \begin{equation*}
          \mathbf{f}_{\text{fused}} = \alpha \cdot \text{Adapter}([\Fv; \mathbf{s}]) + \Fv
        \end{equation*}
      \end{enumerate}
  \end{itemize}
\end{frame}
